...
void Multiply_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Multiply_1_run(uint ctx_index,Circom_CalcWit* ctx);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 18
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 28
// end load src
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 26
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[2]); // line circom 26
...
Multiply_1_create(1,0,ctx,"main",0);
Multiply_1_run(0,ctx);
}
